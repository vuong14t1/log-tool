...
var parserDir = path.normalize(process.cwd() + '/' + basePath + '/parsers/')
var inputDir = path.normalize(process.cwd() + '/' + basePath + '/input/')
var outputDir = path.normalize(process.cwd() + '/' + basePath + '/output/')
...
function LogTool(opts) {
  ...
  */
  ...
  var appConfigFile = path.normalize(process.cwd() + '/' + configPath + '/app.json')
  var inputConfigFile = path.normalize(process.cwd() + '/' + configPath + '/input.json')
  var outputConfigFile = path.normalize(process.cwd() + '/' + configPath + '/output.json')
  var parserConfigFile = path.normalize(process.cwd() + '/' + configPath + '/parsers.json')
  var connectionConfigFile = path.normalize(process.cwd() + '/' + configPath + '/connections.json')
  ...
  for( i=0; i<connectionConfig.length; i++){
    ...
    for(j=0; j<src.length; j++){
      ...
      if(appConfig.inputModules.indexOf(moduleName) < 0) //not an ind. module, in the folder
        ...
        InputModule = require(moduleName)
    ...
    for(j=0; j<dst.length; j++){
      ...
      if(appConfig.outputModules.indexOf(moduleName) < 0) //not an ind. module, in the folder
        ...
        OutputModule = require(moduleName)
